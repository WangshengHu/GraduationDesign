...
struct phrase_set {
  long long size;
  char *phrases[500000000];
} phrase_set;

int addPhrase(char *word) {
  int i = 0;
  while (i < phrase_set.size) {
    if (!strcmp(word, phrase_set.phrases[i])) {
      return 0;
    }
    i++;
  }
  phrase_set.phrases[phrase_set.size] = (char*)calloc(MAX_STRING, sizeof(char));
  strcpy(phrase_set.phrases[phrase_set.size++], word);
  return 1;
}

char train_file[MAX_STRING], output_file[MAX_STRING], phrases_file[MAX_STRING];
...
void TrainModel() {
  ...
  while (1) {
    ...
    if (score > threshold) {
      fprintf(fo, "_%s", word);
      if(addPhrase(bigram_word)) {
        fprintf(fop, "%s %s %s %lld %lld %lld %f %f %f\n", bigram_word, last_word, word, pab, pa, pb, pab / (real)pa, pab / (real)pb, pab / (real)pa / (real)pb);
      }
      pb = 0;
    } else {
      fprintf(fo, " %s", word);
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf("\t-phrases <file>\n");
    printf("\t\tUse <file> to save the resulting phrases (with freq)\n");
